...
var DecCalculator = function (_Calculator) {
    ...
    _createClass(DecCalculator, [{
        ...
        value: function initEvents() {
            ...
            this.$calculatorDOMElement.find(".operator-bar").on('click', function (event) {
                _this2.checkNumber();
            });
        }
    }, {
        ...
        key: 'updateResult',
        value: function updateResult() {
            _get(DecCalculator.prototype.__proto__ || Object.getPrototypeOf(DecCalculator.prototype), 'updateResult', this).call(this);
            var root = this.$calculatorDOMElement;
            var $resultNumber = root.children(".group-number").children(".result-bit");

            for (var i = this.resultNumberArray.length - 1, j = 0; i >= 0; i--, j++) {
                var valueResult = $($resultNumber).eq(j).find('.active').text(this.resultNumberArray[i]);
            }
        }
    }, {
        key: 'add',
